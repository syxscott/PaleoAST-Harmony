...
export class StateManager {
  ...
  undo(): boolean {
    if (!this._u.length) return false;
    if (this._d) this._r.push(this._d);
    this._d = this._u.pop() ?? null;
    this._m = true;
    return true;
  }

  redo(): boolean {
    if (!this._r.length) return false;
    if (this._d) this._u.push(this._d);
    this._d = this._r.pop() ?? null;
    this._m = true;
    return true;
  }
